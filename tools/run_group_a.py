...
def build_steps(a) -> list[Step]:
    ...
    if "combo" in set(a.mode) and a.sam3_text:
        ...
        steps += [
            ...
            #   🔴🔴 ~~«`--input-scale 0.5` 은 1단계가 결정론(17/17 바이트 동일)»~~ 은 **철회됐다**
            #   (§38-9c → §35-2l-6): **4회로 늘리니 6쌍 중 1쌍만** 우연히 같았고, 재실행 잡음 크기는
            #   **두 배율이 사실상 같다**(coarse ‖Δt‖ 0.583 ↔ 0.407mm · refined 0.118 ↔ 0.121mm).
            #   → **«결정론인 설정» 을 찾지 말고 늘 «재실행 잡음 바닥» 을 같이 잰다**(교훈 #86).
            #   비용 0(파일 병합).
