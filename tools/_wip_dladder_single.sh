...
for Z in 0.30 0.40 0.45 0.50 0.55 0.60 0.70 0.85; do
  T=$(echo $Z | tr -d '.'); C=runs/S44S_$T
  [ -f runs/S44S_${T}_st/frame_0000/meta_stereo.json ] || \
    envs/stereo_onnx/bin/python -m spatial_vision.stages.stereo_onnx --in $C \
      --out runs/S44S_${T}_st --scale 0.5 --model $MODEL 2>&1 | tail -1
  [ -f runs/S44S_${T}_seg/frame_0000/det.json ] || \
    envs/seg_sam3/bin/python -m spatial_vision.stages.segment_sam3 --in $C \
      --out runs/S44S_${T}_seg --target full --prompt "$PROMPT" --confidence 0.05 \
...
for Z in 0.30 0.40 0.45 0.50 0.55 0.60 0.70 0.85; do
  T=$(echo $Z | tr -d '.')
  for R in 1 2; do
    O=runs/S44S_${T}_p$R; [ -f "$O/meta_pose.json" ] && continue
    envs/pose/bin/python -m spatial_vision.stages.pose_fp --in runs/S44S_$T --out $O --obj $OBJ \
      --masks runs/S44S_${T}_seg --depth stereo --depth-dir runs/S44S_${T}_st \
