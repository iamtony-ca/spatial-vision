...
| **② 평행이동 갱신 보폭만 «물체 지름» 에 비례한다(회전은 상수)** | 🟡 **절반** — 회전 상수 **20.000000° 는 논문 p14 값 그대로 쓰인다**. 평행이동은 논문 값(0.02/0.02/0.05m)이 `trans_normalizer` 로 **config 에 있으나 꺼져 있고** 대신 지름 정규화가 탄다 — **그 선택의 이유가 논문에 없다** | ✅ `predict_pose_refine.py:228-229` vs `:221` | **B — 코드만** |
...
★★ **정밀화 (2026-09-03)** — 다만 **논문의 값 자체는 config 에 있다.** `trans_normalizer: [0.02, 0.02, 0.05]`
가 논문 p14 의 학습 교란 *"0.02m, 0.02m, 0.05m"* 와 **정확히 같다.** 그런데 `normalize_xyz: true` 라
**그 경로가 실행되지 않고** `× mesh_diameter/2` 가 탄다(§2.5).
→ 🔴 정확한 서술은 *"논문에 없는 값을 썼다"* 가 아니라 ***"논문의 값을 쓰지 않기로 한 배포 설정이고,
그 선택의 이유가 논문에 없다"*** 다. 회전 쪽은 반대로 `rot_normalizer = 20.000000°` 로 **논문 값이 그대로 쓰인다.**
...
★★★ **결과: stage2 는 회전을 «고치기» 는 해도 «찾지» 는 못한다.**
⚠️ **회전을 «안 낸다» 는 뜻이 아니다** — `pose_refined.json` 에 `R` 이 있고 실측으로
stage1 의 `R` 을 **중앙 0.439°** 움직인다(12/12 프레임). 없는 것은 **«후보를 여러 개 만들어
채점해 고르는» 단계**다. 즉 **국소 정련만** 한다. 1회 회전 보폭이 ±20°로 묶여 있고
...
★★ **정밀화 (2026-09-03)** — 논문이 말한 평행이동 교란 **0.02 / 0.02 / 0.05 m** 는 배포 config 에
**`trans_normalizer: [0.02, 0.02, 0.05]` 로 그대로 있다.** 다만 `normalize_xyz: true` 라
`predict_pose_refine.py:196` 의 그 경로가 **실행되지 않고** `:199 → :229`(`× mesh_diameter/2`)가 탄다.
→ 🔴 **«논문에 없는 값» 이 아니라 «논문의 값을 쓰지 않기로 한 배포 설정»** 이다. 회전 쪽은 반대로
`rot_normalizer = 0.3490658503988659 rad = 20.000000°` 로 **논문 값이 그대로 쓰인다.**
⚠️ 그리고 그 상수는 **refiner(`2023-10-28-18-33-37`)에만 있다** — scorer(`2024-01-11-20-02-45`)에는
없는 것이 정상이다(pose 델타를 안 내므로). `predict_pose_refine.py:97` · `predict_score.py:120` 이
각 `run_name` 을 하드코딩한다.
