...
| 107 | 실물 28cm 에서 팔별 대실패가 **10 / 8 / 5 (RH1 / RH2 / CHULL)** 로 나오자 «CHULL 이 강건하다» 는 서열로 읽었다(§44-2) | 🔴 **같은 설정 두 런이 «거의 서로소인» 실패 집합을 냈다** — `fp_c075` {9,11,12,16,17,29,32,33,35,38} ↔ `fp_chull` {4,13,19,21,35}, **교집합 1개**. 코드를 보니 `flange_mask_proj` 는 `if est2 is not None:` 안에서만 읽히고(`pose_fp.py:388`) `fp_chull` 도 `--input-scale 0.75` 라(`run_group_a.py:489`) **coarse 설정이 비트 단위로 같다.** 잘린 bbox 때문에 초기값이 **수렴 분지 경계**에 있고 거기서는 GPU 비결정성이 프레임의 운명을 정한다 | ★ **교훈 #86 의 짝 — «같다» 뿐 아니라 «다르다» 도 n=40 한 벌로는 못 보인다.** ★★ **판정법이 있다: 실패 «집합» 이 서로소인지 본다.** 설정 효과면 실패 프레임이 **겹쳐야** 하고(어려운 프레임은 어느 설정에서도 어렵다), 서로소면 **잡음**이다. 비율만 비교하면 이 구분이 안 된다. 🔴 그리고 **비교 전에 «그 플래그가 그 단계에 실제로 닿는가» 를 코드로 확인한다** — 이번엔 그럴듯한 기전(§38-10)까지 만들어 붙였는데 설정이 그 단계에 도달하지도 않았다. 「기전이 방향을 예측한다」를 **유의성 부족의 보완재로 쓰면 안 된다** |
...
- `Utils.py:605` + `config.yml` — 배포 자산 실측: `full.ply` **560.6mm → 4.204 mm/px** ↔
  `top_flange.ply` **183.1mm → 1.373 mm/px** (**3.06배**).
...
**기전** — 28cm 에서는 pose 원점(flange 상면 중심) 규약 때문에 **몸체가 화면 밖으로 잘린다**(§35-2h: `n25` 는 20/20 프레임이 잘린다).
...
바꿀 자리는 **두 곳**: `contracts.py:201`(`score_frac: float = 0.9`) ·
`segment_sam3.py:263`(`--select-score-frac` 기본). 🔴 `segment_sam6d.py` 는 `score_frac` 을 **안 넘기므로**
`contracts.py` 기본값을 그대로 따른다 — **한쪽만 고치면 두 백엔드가 달라진다.**
...
- **KPI 는 그 곱이라 «중간» 에서 최대** → **0.588m 가 최적**(104/120).
...
| **거리** | **≈0.59m** — KPI 104/120 로 4거리 중 최대 |
🔴 **«top-1» 은 «프롬프트·거리» 에서만 나온다. 팔은 여전히 계열까지만** 갈린다.
...
🔴 **그래도 기본값은 안 바꿨다**(사용자 결정 대기). 바꾸려면 세 곳이다 —
`contracts.select_index(score_frac=0.9)` · `segment_sam3 --select-score-frac` 기본값 · `tools/run_group_a.py` 의 호출.
