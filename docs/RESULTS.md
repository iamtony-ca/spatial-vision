...
| **`CONFLUENCE_PIPELINE.md`** | **사내 공유용 전체 정리**(설계 근거 → sim 검증 → 실물 검증 → 한계) | 공유·발표할 때 |
...
- `pose_fp.py:388` — `flange_mask_proj` 는 **`if est2 is not None:` 블록 안에서만** 읽힌다 = **stage2 전용**.
- coarse 는 `pose_fp.py:374` `est1.register(...)` 로 두 런이 **비트 단위로 같은 설정**이다.
...
|---|---|---|---|
| `--est-iter` | 5 | **5** (`run_demo.py:20` `est_refine_iter` · `estimater.py:159` `def register(…, iteration=5)`) | `est1.register` (`pose_fp.py:374`) |
| `--refine-iter` | **5** | 🔴 **2** (`run_demo.py:21` `track_refine_iter`) | `est2.track_one` (`pose_fp.py:416`) |
...
- **여전히 휴리스틱이다.** upstream 근거는 그대로 없고(등급 D), *"0.3 이 왜 옳은가" 의 기전*도 없다.
  **«나쁜 값을 고친 것»** 이지 **«원리를 얻은 것»** 이 아니다. 원리적 해법은 여전히 **박스 프롬프트**(§6.5)다.
