...
| M6 | 실환경 | ⬜ | 하드웨어 미확보. 배경·재질 randomization 선행 권장 |
...
> 🛠 **측정 절이 아니다** — 전 절을 가로지르는 **오류 패턴 88건**. 새 실험 설계 전에 훑을 것.
...
통계 한 벌 **§35-2d** · 참조 시트 **§35-2e** · **실험 노트·거리 삼각 대조 §35-2l** ·
...
## ★★★ 35-2l. 실험 노트와 **거리 삼각 대조** — 시행착오를 위한 산출물 (2026-08-19)
...
### ★★ 35-2l-3. 거리 삼각 대조 — GT 없이 **z 편향**을 잡는다
...
→ **텍스트 경로의 기본값은 0.05 로 내려도 된다.** 놓친 프레임만 살아나고 오선택은 안 늘었다.
⚠️ 단 이건 **sim 몸체가 검정이고 프롬프트가 `"black plastic box"`** 라 잘 맞은 조건이다. 실물 몸체가
...
| GT-free 지표 | 없음 | 좌우 일관성 · 후퇴율 · 거리 삼각 · 이상 프레임 · 반복도 |
