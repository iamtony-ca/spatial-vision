...
- **실행 편차 아니다** — 같은 명령 3회 반복이 **마지막 자리까지 동일**하다(FoundationPose 결정론적).
...
> 🛠 **측정 절이 아니다** — 전 절을 가로지르는 **오류 패턴 82건**. 새 실험 설계 전에 훑을 것.
...
- 🔴🔴 **가장 약한 축은 «횡방향 평행이동» 이고, 그 이유는 탐색 반경이 아니다.** 횡 1mm 는
  **2.3px** 로 ±8px 안에 한참 들어오는데도 회수가 안 되고 **R 이 0.251 → 0.568° 로 오염**된다.
  → 평면형 윤곽에서 **횡 이동 ↔ 면외 tilt ↔ Z 가 약한 원근 하에서 축퇴**하기 때문이다.
  정합기가 횡 오프셋을 **tilt 로 설명해 버린다.**
