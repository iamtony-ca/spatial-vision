...
| ② | **평행이동 정밀도** — `full` 메쉬 기준 네트워크 1px = **4.34mm** 구조적 천장 | **2단계 메쉬 교체** `full` → `top_flange` (3.16배) | 🟢 FP 논문 §3.3 + `Utils.py:605` |
...
**결과** — 하이브리드 **ADD 중앙 1.395mm** 로 **어느 단일 단계보다 1.5~2배** 좋고, 그 조건에서 **KPI 10/10**
(`refined` 단독은 9/10). ⚠️ **ADD 를 KPI 5mm 와 직접 견주지 말 것** — ADD 는 `full.ply` 기준이고
KPI 는 flange 좌표계다(§8 머리말). **실물 20/20 프레임 통과**(육안 확인). 🔴 그리고 **KPI 를 실제로 지배하는 것은 ①②③ 이 아니라 ④** 였다 —
...
| §5(나) | R·t 접합 가능 (하이브리드) | 🟢 **FP 논문 §5.3** + `Utils.py:850` |
...
줄자보다 **+10~14mm** 큰 계통 편향이 **미해결**인데 **이 기준점 규약이 후보 중 하나**다(→ §10-7).
...
| ② | **평행이동 정밀도** | `full` 메쉬 기준 **네트워크 1px = 4.34mm** — 거리로 못 낮추는 천장 | §4 |
...
### 8.1 하이브리드가 어느 단일 단계보다 낫다 — **설계 ③ 의 핵심 주장**

§5(가)가 예측하는 «coarse 는 R 이 좋고 t 가 나쁘다 / refined 는 반대» 가 **세 파이프라인에서 동시에** 관측된다:

| 출처 | coarse R / t | refined R / t |
|---|---|---|
| 원거리 `full` (n=120) | **0.549°** / 1.713mm | 0.737° / **1.280mm** |
| 근접 `flange` (n=120) | **0.510°** / **0.928mm** | 0.656° / 1.104mm |
| 실물에서 통과한 체인 3종을 **sim 에서 채점** (n=10) | **0.37~0.50°** / 2.0~2.3mm | 1.36~1.42° / **1.04~1.26mm** |

**부호가 한 번도 뒤집히지 않는다.**
→ 하이브리드 **ADD 중앙 1.395mm** — 어느 단일 단계보다 **1.5~2배** 좋고,
`refined` 단독은 KPI **9/10** 인데 하이브리드는 **10/10**.

### 8.2 설계 ② 의 반증 시험 → **§4 말미** (예측대로 이득이 사라졌다)

### 8.3 설계 ④ 의 거리 사다리 → **§6(다)** (거리 최적점은 분할의 성질이었다)
...
7. 🔴 **`t_z` 에 +10~14mm 계통 편향이 있다** — 실물에서 추정 거리가 줄자보다 일관되게 크다.
