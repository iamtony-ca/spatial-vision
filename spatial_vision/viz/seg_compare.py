...
# ★ 팔레트는 **`overlay_pose.COMBO_COLORS` 하나**를 쓴다 — 두 벌로 두면 같은 팔이 두 시트에서
#   다른 색이 되어 눈이 헷갈린다. 🔴 마스크는 **앞에서부터**, pose 는 **뒤에서부터** 집어
#   **둘이 겹치지 않게** 한다(합쳐서 16개를 넘을 때만 만난다).
try:
    from spatial_vision.viz.overlay_pose import COMBO_COLORS as _PAL
except Exception:                                          # noqa: BLE001
    _PAL = [(255, 136, 0), (13, 200, 0), (0, 162, 255), (153, 0, 200),
            (255, 51, 0), (80, 200, 0), (0, 247, 255), (87, 0, 200),
            (255, 0, 34), (147, 200, 0), (0, 255, 178), (0, 180, 200),
            (136, 0, 255), (200, 0, 13), (162, 255, 0), (0, 200, 153)]
COLORS = list(_PAL)                  # 마스크 — 앞에서부터
POSE_COLORS = list(reversed(_PAL))   # pose  — 뒤에서부터
